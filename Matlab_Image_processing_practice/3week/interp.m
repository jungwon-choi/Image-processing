...
output = recon(img, n, key);                    % 가로(열)에 대하여 복원 수행
output = permute(output,[2 1 3]);   % 이미지의 행열을 서로 대칭시켜 변환
output = recon(output, n, key);               % 세로(행)에 대하여 복원 수행
output = permute(output,[2 1 3]);   % 이미지의 행열을 다시 서로 대칭시켜 원래 이미지로 변환

function [output_] = recon(img_, N, key)     % 이미지를 열에 대하여 N배로 복원을 수행
            ...
            switch key % method에 따라 복원 알고리즘을 선택하여 수행
                ...
                case 1 % 1st Interpolation method
                    ...
                    if j+N >W*N                                         % 이미지 범위를 초과한 경우 
                     ...
                     % x1과 x2 사이의 픽셀에 대하여 1st order Interpolation 수식 적용
                    for k = 1 : N-1            
                ...
                case 4 % Cubic Convolution Interpolation method
                    ...
                    % x1과 x2 사이의 픽셀에 대하여 Cubic Convolution Interpolation 수식 적용
                    for k = 1:N-1
                ...
                case 5 % Lanczos Interpolation method
                    ...
                    a = 3;                  
                    x = zeros(2*a,3);                              % 픽셀 값을  for문으로 입력하기 위한 배열 설정
                    
                    %   x(1)  ...  x(a-1)    x(a)    x(a+1)  ...  x(2a)
                    %                          현재픽셀
                    
                    x(a,:) = output_(i,j,:);                        % 현재 좌표 픽셀값
                    
                    for ll = a-1 : -1 : 1 
                        if j-(a-ll)*N < 0                             % 이미지의 범위를 벗어난 경우
                            x(ll,:) = x(ll+1);                         % 다음 좌표의 픽셀 값과 동일하게 처리 
                        else                                             % 이미지의 범위 내의 경우
                            x(ll,:) = output_(i,j-(a-ll)*N,:);   % 해당 좌표의 픽셀값 선택
                        end
                    end
                    
                    for ll = a+1 : 2*a
                        if j-(a-ll)*N > W*N                            % 이미지의 범위를 벗어난 경우
                            x(ll,:) = x(ll-1,:);                       % 이전 좌표의 픽셀 값과 동일하게 처리 
                        else                                             % 이미지의 범위 내의 경우
                            x(ll,:) = output_(i,j-(a-ll)*N,:);   % 해당 좌표의 픽셀값 선택
                        end
                    end
